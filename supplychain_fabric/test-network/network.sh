...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating Company Branch Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-companybranch.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate Company Branch certificates..."
    fi

    infoln "Creating Stationery Company Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-stationerycompany.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate Stationery Company certificates..."
    fi
    ...
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate Supplier certificates..."
    fi


    infoln "Creating Delivery Company Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-deliverycompany.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate Delivery Company certificates..."
    fi
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/companybranch/msp organizations/fabric-ca/companybranch/tls-cert.pem organizations/fabric-ca/companybranch/ca-cert.pem organizations/fabric-ca/companybranch/IssuerPublicKey organizations/fabric-ca/companybranch/IssuerRevocationPublicKey organizations/fabric-ca/companybranch/fabric-ca-server.db'
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/stationerycompany/msp organizations/fabric-ca/stationerycompany/tls-cert.pem organizations/fabric-ca/stationerycompany/ca-cert.pem organizations/fabric-ca/stationerycompany/IssuerPublicKey organizations/fabric-ca/stationerycompany/IssuerRevocationPublicKey organizations/fabric-ca/stationerycompany/fabric-ca-server.db'
	...
	docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/deliverycompany/msp organizations/fabric-ca/deliverycompany/tls-cert.pem organizations/fabric-ca/deliverycompany/ca-cert.pem organizations/fabric-ca/deliverycompany/IssuerPublicKey organizations/fabric-ca/deliverycompany/IssuerRevocationPublicKey organizations/fabric-ca/deliverycompany/fabric-ca-server.db'

...
installChaincode() {
  ...
  INSTALLED=$(peer lifecycle chaincode queryinstalled | grep "Package ID: ${CC_NAME}_${CC_VERSION}")
  
  if [[ -n "$INSTALLED" ]]; then
    # Chaincode is already installed, skip installation
    successln "Chaincode is already installed on peer0.org${ORG}"
    return 0
  fi

  # Chaincode is not installed, proceed with installation
...
infoln "Installing chaincode on peer0.companybranch..."
installChaincode 1
##infoln "Install chaincode on peer0.stationerycompany..."
installChaincode 2
##infoln "Install chaincode on peer0.supplier..."
installChaincode 3
##infoln "Install chaincode on peer0.deliverycompany..."
installChaincode 4
...
if [[ "$CHANNEL_NAME" == "channel1" ]]; then
	...
	checkCommitReadiness 1 "\"CompanyBranchMSP\": true" 

	## now that we know for sure the org have approved, commit the definition
	commitChaincodeDefinition 1

	## query on the org to see that the definition committed successfully
	queryCommitted 1
	
elif [[ "$CHANNEL_NAME" == "channel2" ]]; then
	## approve the definition for all org
	approveForMyOrg 1
	approveForMyOrg 2
	approveForMyOrg 3

	## check whether the chaincode definition is ready to be committed
	checkCommitReadiness 1 "\"CompanyBranchMSP\": true" "\"StationeryCompanyMSP\": true" "\"SupplierMSP\": true" 
	...
	## check whether the chaincode definition is ready to be committed
	checkCommitReadiness 1 "\"CompanyBranchMSP\": true" "\"StationeryCompanyMSP\": true" "\"SupplierMSP\": true" "\"DeliveryCompanyMSP\": true" 
